...
- 25 秒心跳 ping（保活）
- 服务端 100 秒主动轮换连接（先发 `rotate` 事件再优雅关闭并清理订阅，客户端静默重连；避免平台硬杀连接留下的僵尸订阅堵死配额）
...
EdgeOne Pages 部署 Next.js 项目无需额外改动，但 **Node Functions 默认最大执行时长只有 30 秒**，SSE 长连接会被周期性掐断，因此仓库根目录提供了 `edgeone.json`：
...
- `maxDuration` 可配置范围 10–120 秒，不配置时默认 30 秒（这是 EdgeOne 官方文档中 `edgeone.json` 的配置项）
- 服务端在此基础上于 100 秒主动轮换连接（发送 `rotate` 事件后优雅关闭并清理订阅），客户端收到后静默重连，玩家无感知
- **不要删除 `edgeone.json`**：改回默认 30 秒后，连接每 30 秒被平台强制掐断一次（虽然客户端能自动恢复，但重连频繁、体验差）

部署与验证：

1. EdgeOne Pages 控制台导入 Git 仓库，构建命令保持默认（`npm run build`）即可，无需环境变量
2. 验证：好友对战两人连续对弈 5 分钟以上，DevTools Network 中 `stream` 请求应约每 100 秒轮换一次且无 429；中途断网恢复后棋盘状态自动同步

排查提示：

- 若对局中偶发 `429 Too Many Requests`，先看响应体：内容为 `该房间连接数已达上限` 说明是应用层连接配额（已由"先回收僵尸再拒绝"策略兜底）；若为 EdgeOne 的 HTML 错误页则是平台侧限流，需在控制台检查站点安全 / 速率限制配置
- EdgeOne Pages 的 Node Functions 是无状态实例复用模型，`globalThis` 内存存储只在同一实例内有效：低流量场景（好友对战）通常单实例；跨实例时可能出现"房间不存在"或状态不同步，与该部署形态的限制相同

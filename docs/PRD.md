...
> **文档状态说明**：本文档为项目初期的需求规划（历史版本），其中实时通信（Supabase Realtime/WebSocket）、数据库（Supabase）等方案**未在实际实现中采用**。实际实现为：SSE 长连接 + 进程内内存存储 + 服务端权威走子校验。当前的架构与约定以 [AGENTS.md](../AGENTS.md) 为准，功能与部署说明见 [README.md](../README.md)。
